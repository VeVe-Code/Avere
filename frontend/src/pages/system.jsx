...
import { Search, ZapOff } from "lucide-react"
...
function System() {
  ...
  let [loading, setLoading] = useState(false)   // ✅ ADD

  useEffect(() => {
    let fetchData = async () => {
      try {
        setLoading(true)                         // ✅ ADD
        let res = await axios.get('/api/publicsystems?title=' + search)
        ...
        {!loading && Array.isArray(data) && data.map((d, index) => (
          ...
          >
            ...
            <motion.img
              ...
              className="w-full sm:w-44 md:w-52 h- sm50:h-50 md:h-40
