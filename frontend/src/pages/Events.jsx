...
function Events() {
  ...
  useEffect(() => {
    let fetchData = async () => {
      try {
        setLoading(true)
        let res = await axios.get(`/api/publicevents?title=${search}&page=${page}`)
  ...
  return (
    <section className="min-h-screen relative overflow-hidden bg-white dark:bg-slate-950">
      ...
      <div className="relative z-10 py-14 sm:py-16 px-4 sm:px-6 md:px-10 lg:px-24">
        <SEO title="Events - Avere" description="Upcoming events and gatherings from Avere" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5 mb-10 md:mb-12">
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 max-w-md">
                Gatherings, launches, and community moments.
              </p>
            </motion.div>
