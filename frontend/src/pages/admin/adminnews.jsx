...
function AdminNews() {
  ...
  const {
    ...
    fetchUrl: '/api/news',
    reorderUrl: '/api/news/reorder',
    moveStorageKey: 'admin-move:news',
    catalogKey: 'news',
    adminListPath: '/admin/adminnews',
    page,
    linksKey: 'Links',
  })
  ...
  const onDeleted = (_id) => {
    if (news.length === 1 && page > 1) {
      navigate('/admin/adminnews?page=' + (page - 1))
    } else {
      setNews((prev) => prev.filter((k) => k._id !== _id))
    }
  }
  ...
  return (
    <>
            ...
            {news.length > 0 &&
              news.map((k) => (
                <AdminCatalogCard
                  ...
                  photo={k.photo}
                  detailPath={`/admin/adminnews/${k._id}`}
                  editPath={`/admin/adminnews/edit/${k._id}`}
                  apiBase="/api/news"
      ...
      <div className="flex justify-center mt-10">
        {links && (
          <Pagination basePath="/admin/adminnews" links={links} page={page} />
        )}
      </div>
    </>
  )
