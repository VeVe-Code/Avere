...
function NewsForm() {
  ...
  let createNews = async (e) => {
    ...
    try {
      ...
      let res;
      if (id) {
        res = await axios.patch("/api/news/" + id, payload);
      } else {
        res = await axios.post("/api/news", payload);
      }
      ...
      if (file) {
        ...
        await axios.post(`/api/news/${kid}/upload`, formData, {
  ...
  return (
    <AdminFormCard
      ...
      footer={
        <FormActions
          onCancel={() => navigate("/admin/adminnews")}
          saving={saving}
          isEdit={!!id}
