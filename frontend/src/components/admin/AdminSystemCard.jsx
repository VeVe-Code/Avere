...
function AdminSystemCard({ s, ondeleted, onHiddenChange }) {
  const [busy, setBusy] = useState(false)
  const [confirmOpen, setConfirmOpen] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const isHidden = Boolean(s.hidden)

  let deletedData = async () => {
    try {
      setDeleting(true)
  ...
  const toggleHidden = async () => {
    ...
    setBusy(true)
    try {
      const res = await axios.patch('/api/systems/' + s._id + '/hidden')
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm relative">
      ...
      <ConfirmDialog
        open={confirmOpen}
        title="Delete this system item?"
        message={`“${s.title}” will be permanently removed.`}
        confirmLabel="Yes"
        cancelLabel="No"
        busy={deleting}
        onCancel={() => !deleting && setConfirmOpen(false)}
        onConfirm={deletedData}
      />
    </div>
  )
}
